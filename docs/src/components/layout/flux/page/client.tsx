...
function useTocPopover() {
  const value = use(TocPopoverContext);
  if (!value) {
    throw new Error("Missing TocPopoverContext");
  }
  return value;
}

export function PageTOCPopover(props: ComponentPropsWithoutRef<"div">) {
...
function PageTOCPopoverPhysical({ className, children, ...rest }: ComponentPropsWithoutRef<"div">) {
...
export function PageTOCPopoverTrigger({ className, ...props }: ComponentPropsWithoutRef<"button">) {
  const { text } = useI18n();
...
interface ProgressCircleProps extends Omit<ComponentPropsWithoutRef<"svg">, "strokeWidth"> {
  value: number;
  strokeWidth?: number;
  size?: number;
  min?: number;
...
export function PageTOCPopoverContent(props: ComponentPropsWithoutRef<"div">) {
  return (
    <CollapsibleContent
...
export function PageLastUpdate({
  date: value,
  ...props
}: Omit<ComponentPropsWithoutRef<"p">, "children"> & { date: Date }) {
  const { text } = useI18n();
...
export type BreadcrumbProps = BreadcrumbOptions & ComponentPropsWithoutRef<"div">;
